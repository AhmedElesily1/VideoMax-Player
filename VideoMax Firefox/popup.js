...
function renderBlacklist(lang) {
  ...
  if (blacklist.length === 0) {
    ...
    empty.style.cssText = 'color:var(--text-dim);font-size:12px;text-align:center;padding:10px;';
    empty.textContent = i18n[lang].no_sites;
  ...
  blacklist.forEach(domain => {
    ...
    removeSpan.className = 'remove-domain';
    removeSpan.setAttribute('data-domain', domain);
    removeSpan.textContent = '×';
    dEl.append(nameSpan, ' ', removeSpan);
...
function renderPlaylistPopup(lang) {
  ...
  api.storage.local.get(['up_playlist'], (r) => {
    ...
    if (!list.length) {
      ...
      empty.textContent = i18n[lang].playlist_empty;
      listEl.appendChild(empty);
      return;
    }
    list.forEach((item, i) => {
